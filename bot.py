...
def send_message(recipient: str, msg: str):
    try:
        ...
        logging.error('Seems like the user I tried to message vanished into thin air')
        logging.error(traceback.format_exc())
        ...
        if match and event[0]['invokeruid'] in COMMAND_WHITELIST:
            ...
            try:
                ...
                cur.execute('UPDATE `users` SET `ignored` = TRUE '
                            ...
                            (match.group(1), json.get('name')))
                msqlc.commit()
                logging.info('{} ({}) marked previous instances of {} as ignored'.format(event[0]['invokername'],
                                                                                         event[0]['invokeruid'],
                                                                                         match.group(1)))
                send_message(event[0]['invokerid'],
                             'Done! Rechte von {} vorherigen Nutzern entzogen.'.format(len(results)))
            except msql.Error as err:
                logging.error('Failed to mark api key {} as ignored'.format(match.group(1)))
