    ...
    if event.event == "notifyclientmoved":
        ...
        try:
            ...
            if server:
                    ...
                    if (
                        ...
                        logger.info(
                            "User {} ({}) registered a second time for whatever reason".format(
                                event[0]["invokername"], event[0]["invokeruid"]
                            )
                        )
                        send_message(
                            event[0]["invokerid"],
                ...
                logger.warning(
                    "User {} ({}) is currently on world {} and tried to register.".format(
                        event[0]["invokername"], event[0]["invokeruid"], world
                    )
                )
                send_message(
                    event[0]["invokerid"],
...
if __name__ == "__main__":
    ...
    hldr = logging.handlers.TimedRotatingFileHandler(
        "logs/bot.log", when="W0", encoding="utf-8", backupCount=16
    )
